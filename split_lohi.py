...
def construct_similarity_matrix(smiles, threshold, verbose=False):
    ...
    if verbose:
        mols = tqdm(mols, desc="Generating fingerprints")

    all_fps = [fp_generator.GetFingerprint(mol) for mol in mols]
    num_mols = len(all_fps)

    # Store only values above threshold to optimize memory usage
    rows, cols, data = [], [], []
    if verbose:
        all_fps = tqdm(all_fps, desc="Computing pairwise similarities")
    ...
    return similarity_matrix, num_mols


def select_distinct_clusters(smiles, threshold, min_cluster_size, max_clusters, values, std_threshold, verbose=False):
    """
    A greedy algorithm to select independent clusters from datasets.
    """
    clusters = []
    similarity_matrix, num_mols = construct_similarity_matrix(smiles, threshold, verbose=verbose)
